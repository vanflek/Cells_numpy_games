...
def possible_cells(screen_matrix):
    cells_possible = []
    # corners checking
    ...
    if sum(screen_matrix[TILE_Y - 1, TILE_X - 2:TILE_X] == 1) + sum(
            screen_matrix[TILE_Y - 2:TILE_Y, TILE_X - 1] == 1) == 1:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_r:
                ...
                cells_possible = possible_cells(screen_matrix)
            ...
            if event.key == pygame.K_f:
                while True:
                    screen_matrix[screen_matrix == 2] = 0
                    cells_possible = possible_cells(screen_matrix)
                    if len(cells_possible) == 0:
                        break
                    else:
                        y, x = cells_possible[random.randint(0, len(cells_possible) - 1)]
                        screen_matrix[y, x] = 1
                    print(screen_matrix)
            elif event.key == pygame.K_s:
                numpy.save('maze.npy', numpy.array(screen_matrix))
            elif event.key == pygame.K_l:
                screen_matrix = numpy.load('maze.npy')
                screen_matrix[screen_matrix == 2] = 0
        #control
            ...
            if event.button == 1:
                if red_possible:
                    ...
                    if screen_matrix[pygame.mouse.get_pos()[1]//TILE_SIZE, pygame.mouse.get_pos()[0]//TILE_SIZE] == 2:
                        screen_matrix[pygame.mouse.get_pos()[1] // TILE_SIZE, pygame.mouse.get_pos()[0] // TILE_SIZE] = 1
            if event.button == 3:
